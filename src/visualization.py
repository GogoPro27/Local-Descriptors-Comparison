...
def draw_matches(img1, kp1, img2, kp2, matches, max_matches=50):
    ...
    # Make sure the images are color (3 channel) if you want color matches
    if len(img1.shape) == 2:
...
def draw_thick_matches(img1, kp1, img2, kp2, matches, max_matches=50, line_thickness=2):
    ...
    for match in matches:
        ...
        pt2 = (pt2[0] + img1_color.shape[1], pt2[1])  # Adjust for second image offset
...
def draw_colored_matches(img1, kp1, img2, kp2, matches, max_matches=50, line_thickness=2):
    ...
    # Draw matches with colors based on distance
    ...
    for i, match in enumerate(matches):
        ...
        # Choose color based on distance (gradient)
        normalized_distance = match.distance / max_distance
        gradient_color = (
            int(255 * (1 - normalized_distance)),  # Red decreases with better matches
            int(255 * normalized_distance),        # Green increases with worse matches
            0                                      # No blue for simplicity
        )

        # Or, cycle through predefined colors
        # gradient_color = color_list[i % len(color_list)]
